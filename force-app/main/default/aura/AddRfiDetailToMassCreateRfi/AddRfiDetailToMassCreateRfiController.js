...
previous : function(component, event, helper) {
    ...
    if(checkvalue.length > 0){
        for (var i = 0; i < checkvalue.length; i++){
            if (checkvalue[i].get("v.value") == true){
                ...
                if(wrapperIncorrectCat[i].selOption == 'None'){
                    errorPick++;
                    errorPickString.push(wrapperIncorrectCat[i].lstRfiCat.Name);
                }
    ...
    if(errorPick >0 || errorDes >  0 || errorVal >0 ){
        ...
        if(checkvalue.length > 0){
            ...
            if(checkboxValue){
                selectedCategory[checkvalue.get("v.text")] = JSON.stringify(wrapperIncorrectCat[i]);
            }
        }  
...
next : function(component, event, helper){
    ...
    if(checkvalue.length > 0){
        for (var i = 0; i < checkvalue.length; i++){
            if (checkvalue[i].get("v.value") == true){
                // alert('wrapperIncorrectCat[i].selOption - '+wrapperIncorrectCat[i].selOption);
    ...
    if(errorPick >0 || errorDes >  0 || errorVal >0 ){
        ...
        if(wrapperMissingCat.length > 0){
            ...
            selectedCategory = component.get("v.selectedCategory");
            var wrapperMissingCat = component.get('v.wrapperMissingCat');
        //     var checkValueImplementationBlk= [];
            ///   checkValueImplementationBlk = component.find("implementationBlk");
            // console.log('checkvalue-'+checkvalue.length);
            console.log('wrapperMissingCat'+wrapperMissingCat.length);
            if(checkvalue.length > 0){
                for (var i = 0; i < checkvalue.length; i++){
...
saveNew : function(component, event, helper){
    ...
    if(errorPick >0 || errorDes >  0 || errorVal >0 ){
        ...
        component.set("v.selectedCategory",selectedCategory);console.log('length'+selectedCategory.length)
        component.set("v.showCatMissing",false);
        
        var catValue1 =[];
        catValue1 = Object.keys(selectedCategory);
        console.log('length - '+catValue1.length) ;
